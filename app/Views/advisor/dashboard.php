    ...
    <div class="col-md-12">
        <div class="card shadow-sm border-0">
            <div class="card-header py-3 d-flex justify-content-between align-items-center">
                <h5 class="mb-0"><i class="fas fa-list"></i> รายละเอียดกิจกรรมทั้งหมด</h5>
                <div class="btn-group">
                    <button onclick="exportToExcel('advisorTable')" class="btn btn-success btn-sm"><i
                            class="fas fa-file-excel"></i> Excel</button>
                    <button onclick="window.print()" class="btn btn-danger btn-sm"><i class="fas fa-file-pdf"></i>
                        PDF</button>
                </div>
            </div>
            <div class="card-body">
                            ...
                            <?php foreach ($activities as $act): ?>
                                <tr>
                                    ...
                                    <td class="text-center"><span
                                            class="badge bg-info text-dark"><?= $act['participant_count'] ?> คน</span></td>
                                    <td>
                                        <span
                                            class="badge <?= $act['status'] == 'approved' ? 'bg-success' : 'bg-warning text-dark' ?>">
                                            <?= $act['status'] ?>
                                        </span>
                                    </td>
                                </tr>
...
<script>
    let advisorChart;
    const renderChart = () => {
        const config = window.getChartColors();
